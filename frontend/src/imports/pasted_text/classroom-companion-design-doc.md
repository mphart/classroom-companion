...
Raw uncut transcript text
Edited text with user comments
...
Description
Prerecorded videos
User uploads videos, YouTube links, or audio files — same note features apply
Student Planner
Calendar-style planner where users can attach notes and recordings to events
Slides & Documents
Upload slides or PDFs alongside a recording session
Important Info Alerts
AI flags high-priority mentions (e.g., exam dates, "this will be on the test")
User Side Notes
Separate note-taking tab that can be included in AI tools and exam prep
...
Full Transcript
Key Terms & Definitions (if generated)
Study Questions (if generated)
...
9. Open Questions
The following decisions still need to be resolved before or during development:
#
Question
Impact
1
Will there be a confirm password field on signup?
Small UX decision with password-reset implications
2
Will email be collected at signup?
Required later if password reset is added
3
What is the auth strategy? (JWT recommended)
Security architecture — token storage, expiry, refresh
4
Will the app be deployed? If so, where?
Hosting, CORS config, WebSocket support in production
...
✅ Complete
Viewer Page (Post-Recording)
✅ Complete
Viewer Page (Home Selection)
✅ Complete
...
✅ Complete
API integrations
✅ Deepgram + Cursor API confirmed
Auth strategy
⚠️ JWT recommended, not yet confirmed
Deployment / hosting
⚠️ Not yet decided
Stretch goal architecture
⚠️ Accounted for in data model, not fully specced
